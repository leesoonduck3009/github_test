...
namespace B21520638_Tuan1_Bai1
{
    class Point
    {
    ...
    class Triangle : Shape
    {
        ...
        public override double area
        {
            ...
            {
                double l1, l2, l3;
                l1 = Math.Sqrt((Pb.x - Pc.x) * (Pb.x - Pc.x) + (Pb.y - Pc.y) * (Pb.y - Pc.y));
                l2 = Math.Sqrt((Pa.x - Pc.x) * (Pa.x - Pc.x) + (Pa.y - Pc.y) * (Pa.y - Pc.y));
                l3 = Math.Sqrt((Pa.x - Pb.x) * (Pa.x - Pb.x) + (Pa.y - Pb.y) * (Pa.y - Pb.y));
                double p = (l1 + l2 + l3) / 2;
